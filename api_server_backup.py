# api_server.py - Complete REST API server
...
def home():
    return jsonify({
        ...
        'endpoints': {
            'diagnostics': {
                'engine': '/api/diagnose/engine',
                'battery': '/api/diagnose/battery',
                'starter': '/api/diagnose/starter'
            },
            'customers': {
            ...
            'reports': {
                'generate': '/api/report/generate/<diagnostic_id>',
                'download': '/api/report/download/<filename>'
            },
            'history': '/api/history/<vehicle_id>',
...
def diagnose_engine():
    ...
    # Save to database if vehicle ID provided
    if vehicle_id:
...
if __name__ == '__main__':
    print("\n" + "="*60)
    print(f"   {Config.WORKSHOP_NAME}")
    print("   PROFESSIONAL DIAGNOSTIC SYSTEM")
    print("="*60)
